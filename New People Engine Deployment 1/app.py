...
# Gemini 2.5 Pro Configuration - PEOPLE & OPERATIONS ENGINE KEYS
GEMINI_API_KEYS = [
    "AIzaSyC89MUkCE8pgY6Gi1D0gx4wcdJYOeevlNo",  # Back_Peop01
    "AIzaSyAZdCmINg_Qa2E_9oE_eggKj9Gea2QvHaQ",  # Back_Peop02
    "AIzaSyDXIoILPlcR35G2D3WPPDnqcd3FH4Yp7QI",  # Back_Peop03
    "AIzaSyAqBjaFSSiLwiyjf-pmGvZsUSSpHXuEnpw",  # Back_Peop04
    "AIzaSyCSMHu2acQTyvZkOlI41q-iQ466Qg9tutY",  # Back_Peop05
    "AIzaSyDl9lzzX_N4cQIjXFsKXSZi8vwJMXj03x4",  # Back_Peop06
    "AIzaSyBrYM5_HDzRYaqY8xSPmsHrBSHMEBPFCNw",  # Back_Peop07
    "AIzaSyBXS3XTc23ClYjiGJxcg1VlKuC6AsjKqg4",  # Back_Peop08
    "AIzaSyBnKTdGLPDpQtZbBkhHD8X7mccwyWcIN1I",  # Back_Peop09
    "AIzaSyCOHVdkP9SP9ItrJZvHXONwD-hompU2nKw"   # Back_Peop10
]
...
async def send_people_ops_notification(user_id: str, title: str, body: str, data_type: str = "notification"):
    """Send notification to user for people & operations strategy"""
    try:
        payload = {
            "userId": int(user_id),
            "title": title,
        ...
        logging.info(f"🔔 Sending people & ops notification to user {user_id}: {title}")
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=NOTIFICATION_TIMEOUT)) as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logging.info(f"✅ People & ops notification sent successfully to user {user_id}")
                    return True, result
                else:
...
def generate_comprehensive_people_ops_report(complete_raw_data: Dict, report_id: str, max_report_retries: int = 2) -> Dict:
                    ...
                    try:
                        ...
                        if report_id in people_ops_job_status:
                            ...
                            if not notifications_sent["middle"] and completion_percentage >= 45 and completion_percentage <= 65:
                                ...
                                send_people_ops_notification_background(user_id, user_profile, "middle", progress_data)
    ...
    if not notifications_sent["complete"]:
        ...
        send_people_ops_notification_background(user_id, user_profile, "complete", completion_data)
...
async def upload_people_ops_report_to_azure(report_data: Dict, report_id: str, user_id: str):
    ...
    try:
        ...
        # Create folder structure: people operations report/
        folder_name = "people operations report"
...
async def process_people_ops_audit(user_id: str, request: PeopleOpsAssessmentRequest):
    ...
    try:
        ...
        # Update job status
        people_ops_job_status[report_id]["message"] = "Generating comprehensive people & operations strategy..."
        ...
        # Update job status
        people_ops_job_status[report_id]["message"] = "Uploading people & operations strategy to secure storage..."
        ...
        people_ops_job_status[report_id]["message"] = "Triggering auto-indexer for search optimization..."
        people_ops_job_status[report_id]["progress"] = 90
        
        try:
            # Trigger indexer in background with detailed logging
            indexer_trigger_start = time.time()
            
            logging.info(f"🔄 Launching background indexer thread for report_id={report_id}")
            indexer_thread = trigger_people_ops_indexer_background(report_id, user_id, container_name)
            
            indexer_trigger_time = time.time() - indexer_trigger_start
            
            logging.info(f"✅ INDEXER TRIGGERED SUCCESSFULLY!")
            logging.info(f"📊 Indexer trigger details:")
            logging.info(f"   - Trigger time: {indexer_trigger_time:.3f}s")
            logging.info(f"   - Thread created: {indexer_thread.name if hasattr(indexer_thread, 'name') else 'Unknown'}")
            logging.info(f"   - Thread alive: {indexer_thread.is_alive() if indexer_thread else 'No thread'}")
            logging.info(f"   - Background processing: ACTIVE")
            logging.info(f"   - Expected indexing time: 2-5 minutes")
            
            # Add indexer info to generation metadata
            generation_metadata["indexer_integration"] = {
                "trigger_successful": True,
                "trigger_time": indexer_trigger_time,
                "background_thread_started": bool(indexer_thread),
                "indexer_api_url": INDEXER_API_BASE_URL,
                "indexer_timeout": INDEXER_TIMEOUT,
                "indexer_retry_attempts": INDEXER_RETRY_ATTEMPTS,
                "container_name": container_name,
                "folder_path": "people operations report",
                "files_to_index": 57,  # From upload summary
                "triggered_at": datetime.now().isoformat()
            }
            
            logging.info(f"📝 Added indexer integration metadata to generation report")
            
        except Exception as indexer_error:
            indexer_trigger_time = time.time() - indexer_trigger_start if 'indexer_trigger_start' in locals() else 0
            
            logging.error(f"❌ INDEXER TRIGGER FAILED!")
            logging.error(f"🔍 Indexer error details:")
            logging.error(f"   - Error type: {type(indexer_error).__name__}")
            logging.error(f"   - Error message: {str(indexer_error)}")
            logging.error(f"   - Trigger attempt time: {indexer_trigger_time:.3f}s")
            logging.error(f"   - Report ID: {report_id}")
            logging.error(f"   - User ID: {user_id}")
            logging.error(f"   - Container: {container_name}")
            
            # Log full traceback for indexer errors
            import traceback
            logging.error(f"🔍 Indexer error traceback:")
            for line in traceback.format_exc().split('\n'):
                if line.strip():
                    logging.error(f"   {line}")
            
            # Store indexer failure in database
            try:
                store_people_ops_indexer_job_metadata(
                    report_id=report_id,
                    user_id=user_id,
                    indexer_job_id="trigger_failed",
                    indexer_status="failed",
                    error_message=str(indexer_error)
                )
                logging.warning(f"✅ Stored indexer failure in database")
            except Exception as store_error:
                logging.error(f"❌ Failed to store indexer failure: {store_error}")
            
            # Add failure info to generation metadata
            generation_metadata["indexer_integration"] = {
                "trigger_successful": False,
                "trigger_error": str(indexer_error),
                "trigger_time": indexer_trigger_time,
                "error_type": type(indexer_error).__name__,
                "indexer_api_url": INDEXER_API_BASE_URL,
                "failed_at": datetime.now().isoformat()
            }
            
            # DON'T FAIL THE ENTIRE PROCESS - Report was generated successfully
            logging.warning(f"⚠️ CONTINUING WITHOUT INDEXER: Report generation was successful")
            logging.warning(f"📊 Impact: Files uploaded but not indexed for search")
            logging.warning(f"📊 User can still access their report, just no search functionality")
